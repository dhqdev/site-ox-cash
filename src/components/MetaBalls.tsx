...
const MetaBalls: React.FC<MetaBallsProps> = ({
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < effectiveBallCount; i++) {
      ...
      const baseScale = 8.0 + h1[1] * (15.0 - 8.0); // Maior variação para espalhar
      const h2 = hash33(h1);
      const toggle = Math.floor(h2[0] * 5.0); // Mais variações de movimento
      const radiusVal = 0.4 + h2[2] * (1.8 - 0.4); // Raios mais variados
    ...
    function update(t: number) {
      ...
      for (let i = 0; i < effectiveBallCount; i++) {
        ...
        // Diferentes tipos de movimento baseado no toggle
        switch (p.toggle) {
          case 0: // Circular normal
            x = Math.cos(th);
            y = Math.sin(th);
            break;
          case 1: // Elíptico horizontal
            x = Math.cos(th) * 1.5;
            y = Math.sin(th * 0.7);
            break;
          case 2: // Elíptico vertical
            x = Math.cos(th * 0.8);
            y = Math.sin(th) * 1.4;
            break;
          case 3: // Figura de 8
            ...
            y = Math.sin(th * 2) * 0.5;
            break;
          case 4: // Movimento em zigue-zague
            x = Math.cos(th) + Math.sin(th * 3) * 0.3;
            y = Math.sin(th) + Math.cos(th * 2.5) * 0.3;
            break;
          default: // Movimento linear
            x = Math.cos(p.st) + Math.cos(th) * 0.3;
            y = Math.sin(p.st) + Math.sin(th) * 0.3;
        }
